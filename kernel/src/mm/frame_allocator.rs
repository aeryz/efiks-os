...
static FRAME_ALLOCATOR: SpinLock<FrameAllocator<64>> =
    SpinLock::new(FrameAllocator::new(PhysAddr::ZERO));

/// Initialize the allocator with the given `start_addr`.
pub(super) fn init(start_addr: PhysAddr) {
...
pub fn alloc_frame() -> Result<PhysAddr, Error> {
    FRAME_ALLOCATOR.lock().alloc()
}

#[allow(unused)]
/// Free a single 4k page.
pub fn free_frame(ptr: PhysAddr) {
...
impl<const N: usize> FrameAllocator<N> {
    #[must_use]
    const fn new(start_addr: PhysAddr) -> Self {
        Self {
            start_addr,
            pages: [0; N],
        }
    ...
    #[must_use]
    fn alloc(&mut self) -> Result<PhysAddr, Error> {
        for page_i in 0..self.pages.len() {
            for i in 0..(u64::BITS as usize) {
                if (self.pages[page_i] >> i) & 1 == 0 {
                    ...
                    return Ok(PhysAddr::new(self.start_addr.raw() + alloc_addr));
                }
            }
        }
